class Admin::MeetingController < ApplicationController
  ...
  active_scaffold :meeting do |config|
    ...
    config.columns = [:organizational_unit, :name, :responsible_user, :private_events_password, :description]
    config.list.columns = [:organizational_unit, :name, :responsible_user, :description]
