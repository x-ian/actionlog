class Scaffold::OrganizationalUnitsController < ApplicationController
  # GET /organizational_units
  # GET /organizational_units.xml
  def index
    @organizational_units = OrganizationalUnit.find(:all)
  ...
  def create
    ...
    respond_to do |format|
      if @organizational_unit.save
        flash[:notice] = 'OrganizationalUnit was successfully created.'
        format.html { redirect_to(@organizational_unit) }
  ...
  def update
    ...
    respond_to do |format|
      if @organizational_unit.update_attributes(params[:organizational_unit])
        flash[:notice] = 'OrganizationalUnit was successfully updated.'
        format.html { redirect_to(@organizational_unit) }
        format.xml  { head :ok }
      else
